...
program
  ...
  .command("setup")
  .description(
    "interactive project setup wizard (or non-interactive with --preset)",
  )
  .option(
    "--preset <value>",
    "apply a preset non-interactively (name, key=value pairs, or both)",
  )
  .action(setupCommand);
...
db.command("status")
  .description("show schema info and migration count")
...
env
  .command("sync")
  .description("add missing variables from .env.example")
...
program
  ...
  .command("mcp-server")
  .description(
    "start MCP server for AI assistant integration (Cursor, Claude, Codex)",
  )
  .action(async () => {
